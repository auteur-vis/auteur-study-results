...
export const Task2 = () => {
	...
	useEffect(() => {
		...
		*/
		let countryEmphasis = new autr.Emphasis("Country", ["Afghanistan", "Hong Kong, China", "Greece", "Sweden"]);
		countryEmphasis.include({ name: [ "stroke", "opacity", "label" ]});
		
		let regionEmphasis = new autr.Emphasis("Region", ["Sub-Saharan Africa"]);
		regionEmphasis.include({ name: [ "stroke", "opacity" ]});
		
		let unionEmphasis = countryEmphasis.union(regionEmphasis);
