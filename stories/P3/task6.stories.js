...
import { Draft, Emphasis, Threshold } from "auteur";
...
export const Task6 = () => {
	...
	useEffect(() => {
		...
		*/
		...
		// [firstHalf, secondHalf].forEach((months, i) => {
		// 	draft.augment(
		// 		new Emphasis(
		// 			"Month",
		// 			months,
		...
		const emphasis = new Threshold("Philadelphia", 0, "geq")
		const firstHalfFilter = new Emphasis("Month", firstHalf)
		const secondHalfFilter = new Emphasis("Month", ["Jul", "Aug", "Sep", "Oct", "Nov", "Dec"])
		
		const intersection1 = emphasis.intersect(firstHalfFilter)
		const intersection2 = emphasis.intersect(secondHalfFilter)
		...
		draft
			...
			.x("Month", xScale)
			.y("Philadelphia", yScale)
			.include({ name: ["label", "regression"] })
			.augment(intersection1)
			.augment(intersection2)

		// Couldn't get alignment and colors to work, so I'm just going to move it manually

		const draughty = d3
			.select("#draughty")
			.attr("transform", `translate(${xScale.bandwidth() / 2}, 0)`)
			
		draughty.select("line:nth-of-type(1)")
			.attr("stroke", "red")
		
		draughty.select("line:nth-of-type(2)")
			.attr("stroke", "green")
