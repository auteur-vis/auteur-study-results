...
export const Task2 = () => {
	...
	useEffect(() => {
		...
		*/
		...
		const newEmphasisCountry = new Emphasis("Country", ["Hong Kong, China", "Afghanistan", "Sweden", "Greece"]).selection(scatterpoints);

		const newEmphasisRegion = new Emphasis("Region", "Sub-Saharan Africa").selection(scatterpoints);


		draft.layer(ref.current)
			.x("FertilityRate", xScale)
			.y("LifeExpectancy", yScale)
			.include({ "name": ["stroke", "opacity", "label"] })
			.augment(newEmphasisCountry.getAugs());

		draft.layer(ref.current)
			.x("FertilityRate", xScale)
			.y("LifeExpectancy", yScale)
			.include({ "name": ["stroke", "opacity"] })
			.augment(newEmphasisRegion.getAugs());
