...
export const Task7 = () => {
	...
	useEffect(() => {
		...
		*/
		...
		['Charlotte', 'Seattle'].forEach(c => {
			const style = { "Opacity": { "opacity": "1" }, "fill": { "fill": colorScale(c) } };
			let className = `.temperature${c}`
			let barSelection = svgElement.selectAll(className)
			console.log(barSelection)
			const yThreshold = new Threshold("NewYork", "mean", "geq", style);
			yThreshold.selection(barSelection);
