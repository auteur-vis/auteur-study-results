...
export const Task2 = () => {
	...
	useEffect(() => {
		...
		*/
		...
		const newEmphasis = new Emphasis("Region", "Sub-Saharan Africa");
		const newEmphasis2 = new Emphasis("Country", ["Hong Kong, China", "Afghanistan", "Sweden", "Greece"]);

		newEmphasis.selection(scatterpoints).exclude({"name":["fill", "regression", "label"]});
		newEmphasis2.selection(scatterpoints).exclude({"name":["fill", "regression"]});

		draft.layer(ref.current)
					.x("FertilityRate", xScale)
					.y("LifeExpectancy", yScale)
					.exclude({"name":["fill", "regression"]})
					.augment(newEmphasis.union(newEmphasis2));
		// 
