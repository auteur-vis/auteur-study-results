...
export const Task4 = () => {
	...
	useEffect(() => {
		...
		*/
		...
		let cylinders_levels = Array.from(new Set(data.map(d => d.Cylinders)));

		for (let s of cylinders_levels) {
			var CylindersGroup;
			if (s === "7-8 Cyl.") {
				CylindersGroup = "High";
			} else if (s === "5-6 Cyl.") {
			...
			let speciesSelection = svgElement.selectAll(`.${CylindersGroup}`);
			const speciesStyle = {"regression": {"stroke": colorScale(s)}};
