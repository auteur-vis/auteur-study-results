...
export const Task3 = () => {
	...
	useEffect(() => {
		...
		*/
		...
		const style = {"rect":{"fill":"green", "opacity": 0.2}};
		const newRange = new Range(" PM2_5", [0, 150], "closed", style);

		const style2 = {"rect":{"fill":"yellow", "opacity": 0.2}};
		const newRange2 = new Range(" PM2_5", [150, 300], "closed", style2);

		const style3 = {"rect":{"fill":"red", "opacity": 0.2}};
		const newRange3 = new Range(" PM2_5", [300, 450], "closed", style3);
		...
		draft.layer(ref.current)
			...
			.y(" PM2_5", yScale)
			.include({"name": ["rect"]})
			.augment(newRange.union([newRange2, newRange3]));
		// 
